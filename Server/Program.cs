...
SerialPort serialPort = new SerialPort( "COM9", 115200, Parity.None, 8, StopBits.One );
...
void DataReceivedHandler ( object sender, SerialDataReceivedEventArgs e )
{
    ...
    while ( flag )
    {
        ...
        builder.Append( "0x" + temp.ToString("X2") + " " );
        result [count++] = temp;
        if(temp == (byte)0x03)
            flag = false;
    }
    Console.WriteLine( "Data Received : " + builder );
    if ( result.SequenceEqual( Server.CommunicationPackages.ChannelInspection ))
    {
        Console.WriteLine("ChannelInspection");
    }
}
...
while ( true )
{
    ...
    Console.WriteLine("1- Channel Inspection response");
    Console.WriteLine("2- Analog Inspection response");
    input = Int32.Parse(Console.ReadLine());
    if ( input == 1 )
        SendChannelInspectionResponse();
    else if ( input == 2 )
        SendAnalogInspectionResponse();
}

void SendAnalogInspectionResponse ()
{
    byte [] result = new byte [116];
    Console.WriteLine( "Analog Inspection response sent" );
    result [0] = (byte)0x02;
    result [1] = (byte)0x39;
    result [2] = (byte)0xCA;
    result [3] = (byte)0x36;
    ...
    for ( int i = 0; i < 108; i++ )
    {
        result [count++] = (byte)0x38;
    }
    result [113] = (byte)0x30;
    result [114] = (byte)0x30;
    result [115] = (byte)0x03;
    serialPort.Write(result, 0, 116);
}

void SendChannelInspectionResponse () 
{
    Console.WriteLine( "Channel Inspection response sent" );
    serialPort.Write(new byte[] { 0x02,0x39, 0xC9, 0x30, 0x36, 0x30, 0x38, 0x30, 0x38, 0x30, 0x37, 0x30, 0x30, 0x03 },0,14);
}
...
public static class CommunicationPackages
{
    ...
    private static byte [] CalculateChannelInspection () // start of the conversation
    {
        ...
        byte [] toBeSent = { (byte)EnumCommunicationParameters.Stx,
                                 (byte)EnumCommunicationParameters.Id,
                                 (byte)EnumCommunicationParameters.ChInspection,
                                 (byte)0x30, (byte)0x30, (byte)temp [0], (byte)temp [1],
